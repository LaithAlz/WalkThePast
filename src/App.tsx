...
function Landing({ signedIn, onUpload, onLibrary, onExplore }: { signedIn: boolean; onUpload: () => void; onLibrary: () => void; onExplore: () => void }) {
  return <main className="page landing-page"><Header onUpload={onUpload} onLibrary={onLibrary} showMethod /><section className="landing-hero"><div className="landing-copy"><h1>Learn history<br /><em>by standing in it.</em></h1><p className="lede">Start from a photograph, a document, or a description. We reconstruct the place around it, and mark exactly where the evidence ends and inference begins.</p><div className="actions">{signedIn ? <button className="button" onClick={onUpload}>Create a world</button> : <SignUpButton mode="modal"><button className="button" type="button">Create a world</button></SignUpButton>}<button className="button ghost" onClick={onExplore}>Walk a sample world</button></div></div><div className="hero-visual"><img src={images.petra} alt="Al-Khazneh, the rock-cut Treasury at Petra, seen from the canyon floor" /><div className="image-scrim" /><div className="evidence-chips"><Chip color="green" text="SOURCE-VISIBLE 41%" /><Chip color="amber" text="INFERRED 34%" /><Chip color="purple" text="UNSUPPORTED 25%" /></div></div><p className="caption">SAMPLE WORLD · AL-KHAZNEH · PETRA, JORDAN</p></section><section className="steps" id="method"><Step n="01" title="Add source material" copy="Write a prompt, add images or video, or combine them." /><Step n="02" title="We build it in five minutes" copy="A navigable world, grounded in the evidence you provide." /><Step n="03" title="Walk it, and ask out loud" copy="A voice historian tells you where the evidence ends." /></section></main>;
}
