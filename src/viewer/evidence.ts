...
// dyno's generic helper types are too strict to compose readably, so the block is built untyped.
...
export class EvidenceLayer {
  ...
  attach(mesh: SplatMesh, res: ProvenanceResult) {
    ...
    mesh.objectModifier = D.dynoBlock(
      ...
      ({ gsplat }: { gsplat?: unknown }) => {
        ...
        // Desaturate far more than we darken. Dimming unsupported regions reads as broken
        // lighting across the source-frame boundary; draining colour reads as a deliberate mark.
        const grey = D.mul(D.vec3(lum), f(0.85));
        const fade = D.mul(D.mul(shiftU, D.sub(f(1), visible)), D.sub(f(1), modeU));
        const explored = D.mix(rgb, grey, D.mul(fade, f(0.45)));
  ...
  /**
   * Recompile the tint into the mesh's generator.
   *
   * Spark rebuilds a SplatMesh's generator on its own while the splat data settles after
   * `initialized` resolves, and that rebuild drops the modifier we compiled in `attach()` —
   * `objectModifiers` still holds it, but the running shader no longer applies it, so the
   * world renders untinted. Re-running this puts it back.
   */
  refresh() {
    this.mesh?.updateGenerator();
  }
