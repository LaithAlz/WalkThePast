...
def main() -> None:
    ...
    ap.add_argument("--id", required=True, help="world id -> public/worlds/<id>")
    ...
    ap.add_argument("--out-root", type=Path, default=REPO / "public" / "worlds")
    args = ap.parse_args()
    ...
    manifest = {
        ...
        # Marble's collider mesh is exported in the same raw frame as the splat, so the
        # viewer applies the metric scale, ground offset and axis flip itself ("splat").
        # Without this block the viewer never sees the mesh and falls back to a flat
        # level-ground plane, which reads as flying rather than walking.
        "collider": {"url": "./collider.glb", "space": "splat"} if files.get("collider") else None,
        "bounds": {"radiusM": 3.5},
        "marble": {
            "world_id": world_id,
