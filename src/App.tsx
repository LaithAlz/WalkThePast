...
import { deleteWorld, dismissJob, generateWorld, getCredits, imagineImage, jobImage, listJobs, listWorlds, MODEL_CREDITS, writeGuide, type Job, type MarbleModel, type WorldImage } from "./lib/api";
import { sourceImage, setSessionTokenReader } from "./lib/backend";
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        ...
        // The thumbnail needs the session token, so it arrives as a blob URL, cached per job.
        const list = await Promise.all(listed.map(async (j) => (j.hasImage ? { ...j, image: (await jobImage(j.id)) ?? undefined } : j)));
        if (stop) return;
        setJobs(list);
...
const generatedWorld = (w: { id: string; name: string; image?: string }): SampleWorld => ({ title: w.name, place: "GENERATED WORLD", date: "", evidence: "LIVE PROVENANCE", image: sourceImage(`/worlds/${w.id}/${w.image ?? "source.jpg"}`), note: "Built from a photograph through Marble and classified against it.", quote: "You’re standing where the photographer stood.", worldId: w.id });

const images = {
  atget: "/assets/atget-paris.jpg",
  petra: "/assets/petra-al-khazneh.jpg",
...
export default function App() {
  ...
  // Generation runs in the background on the server: the library shows it
  // building, with a percentage. The guide was chosen on the create screen
  // before Generate was ever pressed, so there is nothing to stop for here.
  const startGeneration = () => setScreen("library");
  const openGenerated = (worldId: string, name: string, image?: string) => {
    setActiveSample({ title: name, place: "YOUR PHOTOGRAPH", date: "", evidence: "LIVE PROVENANCE", image: sourceImage(`/worlds/${worldId}/${image ?? "source.jpg"}`), note: "Generated from your photograph and classified against it.", quote: "You’re standing where the photographer stood.", worldId });
  ...
  useEffect(() => {
    ...
    void listWorlds().then((list) => {
      ...
      if (w) openGenerated(w.id, w.name, w.image); else setScreen("landing");
    });
  }, [screen]); // eslint-disable-line react-hooks/exhaustive-deps

  if (screen === "upload") return <Upload onBack={() => setScreen("landing")} onGenerate={startGeneration} onLibrary={() => setScreen("library")} />;
...
function Header({ onUpload, onLibrary }: { onUpload?: () => void; onLibrary?: () => void }) {
  const { isSignedIn } = useAuth();
  const signedOut = !isSignedIn;
  return <header className="site-header"><Brand /><nav>{!signedOut && onUpload && <button onClick={onUpload}>New world</button>}{!signedOut && onLibrary && <button onClick={onLibrary}>Library</button>}{signedOut ? <><SignInButton mode="modal"><button className="auth-login" type="button">Log in</button></SignInButton><SignUpButton mode="modal"><button className="button compact" type="button">Sign up</button></SignUpButton></> : <UserButton />}</nav></header>;
}

function Landing({ signedIn, onUpload, onLibrary, onExplore }: { signedIn: boolean; onUpload: () => void; onLibrary: () => void; onExplore: () => void }) {
  return <main className="page landing-page"><Header onUpload={onUpload} onLibrary={onLibrary} /><section className="landing-hero"><div className="landing-copy"><h1>Learn history<br /><em>by standing in it.</em></h1><p className="lede">Start from a photograph, a document, or a description. We reconstruct the place around it, and mark exactly where the evidence ends and inference begins.</p><div className="actions">{signedIn ? <button className="button" onClick={onUpload}>Create a world</button> : <SignUpButton mode="modal"><button className="button" type="button">Create a world</button></SignUpButton>}<button className="button ghost" onClick={onExplore}>Walk a sample world</button></div></div><div className="hero-visual"><img src={images.petra} alt="Al-Khazneh, the rock-cut Treasury at Petra, seen from the canyon floor" /><div className="image-scrim" /><div className="evidence-chips"><Chip color="green" text="SOURCE-VISIBLE 41%" /><Chip color="amber" text="INFERRED 34%" /><Chip color="purple" text="UNSUPPORTED 25%" /></div></div><p className="caption">SAMPLE WORLD · AL-KHAZNEH · PETRA, JORDAN</p></section><section className="steps"><Step n="01" title="Add source material" copy="Write a prompt, add images or video, or combine them." /><Step n="02" title="We build it in five minutes" copy="A navigable world, grounded in the evidence you provide." /><Step n="03" title="Walk it, and ask out loud" copy="A voice historian tells you where the evidence ends." /></section></main>;
}
...
function Upload({ onBack, onGenerate, onLibrary }: { onBack: () => void; onGenerate: (gen: Generation) => void; onLibrary: () => void }) {
  ...
  // Prepare the photograph as soon as it is attached, so its size is known before anyone spends credits.
  useEffect(() => { if (imageFiles.length && !prepped) void preparePhoto().catch(() => undefined); }, [attachments, trim]); // eslint-disable-line react-hooks/exhaustive-deps
  const lowRes = prepped && Math.min(prepped.width, prepped.height) < 1024 ? `${prepped.width} × ${prepped.height}` : null;
...
function Library({ onNew, onOpen }: { onNew: () => void; onOpen: (worldId: string, name: string, image?: string) => void }) {
  const [generated, setGenerated] = useState<{ id: string; name: string; image?: string }[]>([]);
  ...
  const ready: LibraryWorld[] = generated.map((w) => ({ id: w.id, title: w.name, detail: "GENERATED WORLD · WALKABLE", image: sourceImage(`/worlds/${w.id}/${w.image ?? "source.jpg"}`), open: () => onOpen(w.id, w.name, w.image), remove: () => deleteWorld(w.id) }));
...
function WorldCard({ world, onClick, onRemoved }: { world: LibraryWorld; onClick: () => void; onRemoved: () => void }) {
  ...
  return <div className="world-card-wrap">
    <button className={`world-card ${world.building ? "building" : ""} ${world.failed ? "failed" : ""}`} onClick={onClick} disabled={!!world.building}><div className="world-image"><img src={world.image} alt="" crossOrigin="anonymous" /><span className="status">{world.failed ? "FAILED" : world.building ? `BUILDING · ${world.pct ?? 0}%` : "READY"}</span>{world.building && <i style={{ width: `${world.pct ?? 0}%` }} />}</div><div className="world-info"><strong>{world.title}</strong><span>{error || world.detail}</span></div></button>
...
function Explore({ world, evidence, speaking, autoEnter = false, voice = false, onToggleEvidence, onExit }: { world: SampleWorld; evidence: boolean; speaking: boolean; autoEnter?: boolean; voice?: boolean; onToggleEvidence: () => void; onExit: () => void }) {
  ...
  // What the historian is doing reaches the guide standing in the world, which
  // is how a voice in your ear becomes someone gesturing in front of you.
